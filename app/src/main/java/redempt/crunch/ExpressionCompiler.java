...
class ExpressionCompiler {
	...
	static redempt.crunch.CompiledExpression compile(String expression, EvaluationEnvironment env) {
		if (expression == null || env == null) {
			throw new ExpressionCompilationException("Expression and environment may not be null");
		}
		redempt.crunch.CompiledExpression exp = new redempt.crunch.CompiledExpression();
		Value val = compileValue(expression, exp, env, 0, false).getFirst();
		exp.setValue(val);
		return exp;
	}
	
	private static Pair<Value, Integer> compileValue(String expression, redempt.crunch.CompiledExpression exp, EvaluationEnvironment env, int begin, boolean parenthetical) {
		...
		for (i = begin; i < expression.length(); i++) {
			...
			if (namedToken.getFirst() != null) {
				...
				if (token.getType() == TokenType.VARIABLE) {
					redempt.crunch.Variable var = ((redempt.crunch.Variable) token).getClone();
					var.expression = exp;
					token = var;
				}
				if (!op && tokenStart != i) {
	...
	private static Pair<ArgumentList, Integer> compileArgumentList(String expression, redempt.crunch.CompiledExpression exp, EvaluationEnvironment env, int start) {
		...
		Value[] valueArray = values.toArray(new Value[0]);
